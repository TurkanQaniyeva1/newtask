...
function change(){
    users.forEach((item)=>{
        ...
        let phonesplit = newphone.split("");
        let countrycode = newphone.split("")[0];
        let newnumber = phonesplit[1].split("").length - 2;
        let change = phonesplit[1].slice(0, newnumber).split("")
        let newdata = change.map((item)=> item >=0 ? "*" : item).join(" ") ;
        let lastnumber = phonesplit[1].slice(newnumber)
        item.phone = countrycode+ "" +newdata +lastnumber
        console.log(item);
    })
}
...
//2
function getPhone(users) {
    let username = prompt("Istifadeci adi daxil edin:");

    for (let i = 0; i < users.length; i++) {
        if (users[i].username === username) {
            alert("Telefon nomreniz: " + users[i].phone); 
            return;
        }
    }
...
let filterUser = users.filter(user => parseInt(user.birthDate.split('-')[0]) >= 1990);
...
function getmail(){
    users.forEach((item) => {
        let newelement = item.email.split('@')[0];
        let newmail = newelement + "@gmail.com";
        item.email = newmail;
...
const eyeColor = () => {
    ...
    users.forEach((item) => {
        ...
        if (detail[eyeColor]) {
            detail[eyeColor].push(item.firstName + " " + item.lastName);
        } else {
            detail[eyeColor] = [item.firstName + " " + item.lastName]; 
        }
    });
    console.log(detail); 
}
